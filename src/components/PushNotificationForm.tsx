"use client"; import React, { useState, useEffect } from 'react'; import { Send, Image, Users, Settings, Loader2, CheckCircle, AlertCircle } from 'lucide-react'; import { toast } from 'react-hot-toast'; import { motion } from 'framer-motion'; interface NotificationStats {  total_count: number;  successful: number;  failed: number;  errored: number; } export default function PushNotificationForm() {  const [title, setTitle] = useState('');  const [message, setMessage] = useState('');  const [imageUrl, setImageUrl] = useState('');  const [actionUrl, setActionUrl] = useState('');  const [sending, setSending] = useState(false);  const [stats, setStats] = useState<NotificationStats | null>(null);  const [loadingStats, setLoadingStats] = useState(false);  // Load OneSignal stats  const loadStats = async () => {    setLoadingStats(true);    try {      const response = await fetch('/api/onesignal?action=app_stats');      const result = await response.json();      if (result.success) {        setStats({          total_count: result.data.players || 0,          successful: 0,          failed: 0,          errored: 0        });      }    } catch (error) {      console.error('Error loading stats:', error);    } finally {      setLoadingStats(false);    }  };  // Send notification  const sendNotification = async () => {    if (!title.trim() || !message.trim()) {      toast.error('Judul dan pesan harus diisi');      return;    }    setSending(true);    try {      const notificationData = {        title,        message,        ...(imageUrl && { big_picture: imageUrl }),        ...(actionUrl && { url: actionUrl }),      };      const response = await fetch('/api/onesignal', {        method: 'POST',        headers: {          'Content-Type': 'application/json',        },        body: JSON.stringify({          action: 'send_notification',          data: notificationData,        }),      });      const result = await response.json();      if (result.success) {        toast.success('Notifikasi berhasil dikirim!');        setTitle('');        setMessage('');        setImageUrl('');        setActionUrl('');        loadStats(); // Refresh stats      } else {        throw new Error(result.error?.errors?.[0] || 'Gagal mengirim notifikasi');      }    } catch (error) {      console.error('Error sending notification:', error);      toast.error(Gagal mengirim notifikasi: ${error}`);
} finally {
setSending(false);
}
};
