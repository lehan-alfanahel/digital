...
export default function MonthlyAttendanceReport() {
 ...
 // Helper function to download file in webview-compatible way
 const downloadFileInWebView = (blob: Blob, filename: string, mimeType: string) => {
   try {
     // Create object URL
     const url = URL.createObjectURL(blob);

     if (isWebView()) {
       // For webview, open in new window/browser
       ...
       // Also try to open in external browser
       setTimeout(() => {
 ...
 const handleDownloadPDF = () => {
   ...
   try {
     ...
     // Generate PDF as blob instead of direct download
     const fileName = `Rekap_Kehadiran_${formattedMonth.replace(' ', '_')}.pdf`;
     const pdfBlob = doc.output('blob');

     // Use webview-compatible download method
 ...
 const handleDownloadExcel = async () => {
   ...
   try {
     ...
     // Generate Excel as blob instead of direct file write
     const excelBlob = new Blob([XLSX.write(wb, { bookType: 'xlsx', type: 'array' })], {
       type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
     });

     // Use webview-compatible download method
     downloadFileInWebView(excelBlob, fileName, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
     ...
     <div className="bg-white rounded-xl shadow-sm p-2 mb-6">
       ...
       <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
         <div className="text-center mt-1 p-4">
           <h2 className="text-gray-800 text-xl font-bold uppercase">{schoolInfo.name}</h2>
           <p className="text-gray-800 text-sm font-bold">{schoolInfo.address}</p>
           <p className="text-gray-800 text-sm font-bold">NPSN {schoolInfo.npsn}</p>
         </div>
         <hr className="border-t border-gray-300 mt-1 mb-5" />
